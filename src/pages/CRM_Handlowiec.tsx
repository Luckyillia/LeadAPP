...
import {
  ...
  Table, Th, Td, Button, SourceBadge,
} from '@/components/ui';
import { QuickAddLeadForm } from '@/components/QuickAddLeadForm';
import { formatDate } from '@/lib/utils';
import { Users, Calendar, Briefcase, TrendingUp, Plus } from 'lucide-react';
...
export function CRM_Handlowiec() {
  ...
  const myLeads = MOCK_LEADS.filter(l =>
    l.source === 'sales_direct' ||
    (user?.role === 'admin')
  );

  const stats = {
    total: myLeads.length,
    pipeline: myLeads.filter(l => l.status === 'pending_direct').length,
    approved: myLeads.filter(l => l.status === 'approved').length,
    value: myLeads.filter(l => l.status === 'approved').length * 850,
  };
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-5 gap-3 mb-6">
        {PIPELINE_STAGES.map((stage, i) => {
          const stageLeads = myLeads.filter(l =>
            (pipelineStage[l.id] ?? 0) === i
          );
          return (
      ...
      {/* Leads table with notes */}
      <h2 className="text-lg font-bold mb-3 flex items-center gap-2">
        <Users className="w-5 h-5 text-emerald-500" />
        Moje Kontakty ({myLeads.length})
      </h2>
      <Table>
        <thead>
          <tr>
