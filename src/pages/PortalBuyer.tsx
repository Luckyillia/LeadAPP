...
import { ShoppingBag, AlertTriangle, Clock, Lock, Eye, Loader2 } from 'lucide-react';

export function PortalBuyer() {
  const { user } = useAuthStore();
  const { data: leads = [], isLoading: leadsLoading } = useBuyerLeads(user?.id ?? '');
  const { data: rejections = [], isLoading: rejLoading } = useBuyerRejections(user?.id ?? '');
  ...
  const stats = {
    ...
    available: leads.filter((l) => !rejectedIds.has(l.id)).length,
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-amber-700 mt-0.5">
              ...
              Po upływie tego czasu faktura staje się wiążąca.
            </p>
          </div>
        </div>
      </Card>
      ...
      {leads.length === 0 ? (
        ...
        <Table>
          <thead>
            <tr><Th>Kontakt</Th><Th>Firma</Th><Th>Dane Kontaktowe</Th><Th>Status</Th><Th>Czas</Th><Th>Reklamacja</Th></tr>
          </thead>
          <tbody>
            {leads.map((lead) => {
              ...
              return (
                <tr key={lead.id} className={`hover:bg-muted/30 transition-colors ${isRejected ? 'opacity-50' : ''}`}>
                  <Td>
                    ...
                    {isRejected && <span className="text-xs text-red-600 font-mono">⚠ Reklamacja złożona</span>}
                  </Td>
                  <Td><span className="text-sm">{lead.companyName}</span></Td>
                  <Td>
                    <div className="space-y-0.5">
                      <div className="text-xs font-mono">
                        {revealed ? lead.email : maskEmail(lead.email)}
                      </div>
                      <div className="text-xs font-mono">
                  ...
                  <Td>
                    ...
                    {expired && <p className="text-xs text-muted-foreground font-mono">Termin minął</p>}
                  </Td>
                  <Td>
                    {isRejected ? (
